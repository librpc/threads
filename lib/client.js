...
const { peekTransferables, uuid } = require('./utils.js')
...
class RpcClient extends EventEmitter {
  ...
  listen (worker) {
    ...
    worker.on('error', this.catch)
  }

  /**
   * Message handler
   * @param {Object} res          Message event data
   * @param {number} res.uid      Remote call uid
   * @param {string} [res.error]  Error discription
   * @param {string} [res.method] Remote procedure name
   * @param {string} [res.event]  Server event name
   * @param {*}      [res.data]   Procedure result or event data
   * @protected
   */
  handler (res) {
    var { uid, error, method, eventName, data } = res
    if (error) {
